...
class MCPAuthorizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not request.url.path.startswith("/mcp"):
            logger.debug(f"[{request_id}] 非MCP端点，跳过中间件: path={request.url.path}")
            return await call_next(request)

        logger.info(f"[{request_id}] MCP端点请求: path={request.url.path}, method={request.method}")
        
        # 如果认证未启用，直接放行
        if not self.auth.enabled:
            logger.warning(f"[{request_id}] 认证未启用，直接放行MCP请求: path={request.url.path}")
            return await call_next(request)
        ...
        logger.debug(f"[{request_id}] Authorization header: {authorization[:30] if authorization else 'None'}...")

        if not authorization or not authorization.startswith("Bearer "):
            logger.warning(f"[{request_id}] MCP 端点认证失败: 缺少Bearer Token, path={request.url.path}")
            return JSONResponse(
                {"jsonrpc": "2.0", "error": {"code": 401, "message": "JWT Token required for MCP endpoints"}},
                status_code=401
            )

        token = authorization.split(" ", 1)[1]
        logger.debug(f"[{request_id}] 提取到JWT Token: {token[:30]}..., 长度={len(token)}")
        ...
        if not payload:
            logger.warning(f"[{request_id}] MCP 端点无效 JWT Token: token={token[:30]}...")
            return JSONResponse(
                {"jsonrpc": "2.0", "error": {"code": 401, "message": "Invalid or expired JWT Token"}},
                status_code=401
            )
        ...
        try:
            ...
            if request.method == "POST":
                ...
                try:
                    ...
                    logger.info(f"[{request_id}] MCP请求: method={method}, id={request_id_str}")
                    ...
                    if method == "tools/list":
                        ...
                        return await self._filter_tools_list_response(request_id, response, user_info)
                    
                    elif method == "tools/call":
                        # 工具调用请求需要权限检查
                        tool_name = request_data.get("params", {}).get("name")
                        logger.info(f"[{request_id}] 工具调用请求: tool={tool_name}, user={user_info.get('name')}, role={user_info.get('role')}")
                        
                        if tool_name:
                            logger.debug(f"[{request_id}] 开始检查权限: role={user_info['role']}, tool={tool_name}")
                            has_permission = self.auth.jwt_utils.check_permission(user_info["role"], tool_name)
                            logger.debug(f"[{request_id}] 权限检查结果: {has_permission}")
                                ...
                                return JSONResponse(
                                    {
                                        "jsonrpc": "2.0",
                                        "id": request_id_str,
                                        "error": {"code": 403, "message": f"Permission denied for tool: {tool_name}"}
                                    },
                                    status_code=403
                                )
                            else:
                                logger.info(f"[{request_id}] 工具调用权限检查通过: tool={tool_name}, role={user_info['role']}")
                    
                    else:
                        logger.info(f"[{request_id}] 继续处理其他MCP请求: method={method}, 直接放行")
    ...
    async def _filter_tools_list_response(self, request_id: int, response, user_info: Dict[str, Any]):
        """过滤工具列表响应
        ...
        try:
            ...
            return JSONResponse(
                {"jsonrpc": "2.0", "error": {"code": 500, "message": f"Failed to read response: {str(e)}"}},
                status_code=500
            )
        ...
        try:
            ...
            if is_sse:
                ...
                if not json_line:
                    ...
                    return JSONResponse(
                        {"jsonrpc": "2.0", "error": {"code": 500, "message": "Invalid SSE response: no data line"}},
                        status_code=500
                    )
            ...
            return JSONResponse(
                {"jsonrpc": "2.0", "error": {"code": 500, "message": f"Invalid UTF-8 response: {str(e)}"}},
                status_code=500
            )
        except json.JSONDecodeError as e:
            logger.error(f"[{request_id}] JSON解析失败: {e}, 响应体: {response_body[:200]}", exc_info=True)
            return JSONResponse(
                {"jsonrpc": "2.0", "error": {"code": 500, "message": f"Invalid JSON response: {str(e)}"}},
                status_code=500
            )
        except Exception as e:
            logger.error(f"[{request_id}] 处理响应失败: {e}", exc_info=True)
            return JSONResponse(
                {"jsonrpc": "2.0", "error": {"code": 500, "message": f"Failed to process response: {str(e)}"}},
                status_code=500
            )
        
        # 过滤工具列表
        logger.debug(f"[{request_id}] 响应JSON: {json.dumps(response_json, ensure_ascii=False)[:500]}")
        ...
        if "result" in response_json and "tools" in response_json["result"]:
            ...
            for tool in all_tools:
                ...
                logger.debug(f"[{request_id}] 工具过滤: {tool_name} -> {has_permission}")
                if has_permission:
