...
def register_task_results_tools(server):
    ...
    @server.mcp.tool(
        name="get_result",
        description=load_tool_description("get_result"),
    )
        ...
        if result is None:
            ...
            if task_status == "running":
                return {
                    "task_id": task_id,
                    "status": "running",
                    "message": f"Task is still running. Poll again after 60 seconds using get_result('{task_id}')"
                }
            return {
                "task_id": task_id,
                "status": task_status,
    ...
    @server.mcp.tool(
        ...
        description=load_tool_description("get_host_detail"),
    )
