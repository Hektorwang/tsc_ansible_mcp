    ...
    if asyncio.iscoroutinefunction(func):
        ...
        async def async_wrapper(*args, **kwargs) -> Dict[str, Any]:
            try:
                ...
                logger.exception(f"执行 {func.__name__} 时发生错误: {e}，参数: {args}, {kwargs}")
...
class ErrorHandler:
    ...
    @classmethod
    def tsc_tools_install_failed(cls, message: str = "tsc_tools 安装失败") -> Dict[str, Any]:
        """返回 tsc_tools 安装失败错误响应"""
        return create_error_response(
            "TSC_TOOLS_INSTALL_FAILED",
            message
        )
